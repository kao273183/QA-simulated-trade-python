...
log_name_txt = ospath + '/logs/ReportLogName_dubai.txt' 
...
for ordersData in orders:
    balance_path = domain + '/orchid/v1/wallet/balance/' + ordersData[0]
    resp = rs.get(balance_path,headers=headers)
    if resp.status_code == 200:
        respJson = json.loads(resp.text)
        orders[count] += (respJson.get('data')[0].get('available'),)
    ...
    future_order_path = domain + '/orchid/v1/future/' + ordersData[15] + '/position'
    resp = rs.get(future_order_path,headers=headers)
    if resp.status_code == 200:
        respJson = json.loads(resp.text)
        orders[count] += (respJson.get('data').get('total_profit_and_loss'),)
    count += 1
# 创建DataFrame
order_columns = ['用戶ID', '交易對', '方向', '倉位方向', '數量', '保證金', '槓桿', '止盈價格', '止損價格', '價格', '是否模擬', '訂單時間', 'Top One UID', 'Fameex UID', '原倉位單', '新倉位單', '響應內容','交易錢包餘額','佣金錢包餘額','總實現損益']
orders_df = pd.DataFrame(orders, columns=order_columns)

close_columns = ['用戶ID', '平倉時間', 'Top One UID', 'Fameex UID', '響應內容']
...
# 更新響應內容列，記錄錯誤信息
orders_df['響應內容'] = orders_df.apply(lambda row: row['響應內容'] if row['響應代碼'] == '102000' else f"code: {row['響應代碼']}, error: {row['錯誤信息']}", axis=1)
closes_df['響應內容'] = closes_df.apply(lambda row: row['響應內容'] if row['響應代碼'] == '102000' else f"code: {row['響應代碼']}, error: {row['錯誤信息']}", axis=1)
...
# 保存为Excel文件
save_report_path = ospath + '/reportExcel/' + str(args.filename) + 'orders_and_closes_summary.xlsx'
